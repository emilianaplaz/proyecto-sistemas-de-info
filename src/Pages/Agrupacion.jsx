...
const Agrupacion = () => { 
  ...
  const [TipoUs, setTipoUsState] = useState(false);
  ...
  const checkAuthStateRef = useRef(null);
 
  useEffect(() => {
    checkAuthStateRef.current = onAuthStateChanged(auth, (user) => {
      if (user) {
        setTipoUsState(true);
        setCurrentUserEmail(user.email);
      } else {
        setTipoUsState(false);
        setCurrentUserEmail(null);
      }
    });

    return () => checkAuthStateRef.current();
  }, []);
  ...
  return ( 
    ...
    <div className="Clubpage"> 
     <div className="button-container">
        ...
        {TipoUs && (
  <button id="delete-buton" onClick={() => deleteAgrupacionByName(AgrupacionNombre)}>
    Eliminar Grupo
  </button>
)}
